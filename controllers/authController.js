...
export const checkUsernameAvailability = async (req, res) => {
    try {
        ...
        if (error.message.includes('buffering timed out')) {
            return res.status(503).json({ available: false, message: 'Database connection timeout. Please check your DB settings.' });
        }
...
export const registerUser = async (req, res) => {
    ...
    try {
        ...
        if (error.message.includes('buffering timed out')) {
            return res.status(503).json({ message: 'Database connection timeout. Please try again in a few minutes.' });
        }
